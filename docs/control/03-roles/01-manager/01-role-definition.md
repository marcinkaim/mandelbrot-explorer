...
* **Mechanism:** The Manager reacts to **Action Reports (AR)** generated by other agents. It validates the **Chain of Custody** (via `Relates To`) and parses the `Outcome` to determine the next valid state (e.g., advancing from `ACTIVE` to `AUDIT`).
